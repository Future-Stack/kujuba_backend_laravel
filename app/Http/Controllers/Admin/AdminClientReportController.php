...
class AdminClientReportController extends Controller
{
     ...
     */
     public function downloadPdf(Request $request)
     {
         ...
         try {
             ...
             $clientName = str_replace([' ', '/', '\\'], '_', strtolower($reportData['reports']['client']['company_name'] ?? 'clients_summary'));
             $date = date('Y-m-d');
    ...
    public function sendEmail(Request $request)
    {
        ...
        try {
            $client = $request->client_id ? User::where('user_type', 'client')->find($request->client_id) : null;
            $targetEmail = $request->recipient_email 
                ?: ($request->email_override 
                ?: ($request->email 
                ?: ($client?->email)));
            ...
            if (!$targetEmail) {
                return response()->json([
                    'success' => false,
                    'message' => 'Please provide a recipient email or select a client.'
                ], 422);
            }
            ...
            Mail::to($targetEmail)->send(new ClientInspectionSummaryMail($report));
